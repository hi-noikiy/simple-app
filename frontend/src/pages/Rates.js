...
import { getTradeData } from "actions";
...
class Rates extends React.Component {
  ...
  fetchTradeData = () => {
    return lendbookApi.get().then(res => {
      ...
      const tradedPrices = _.map(
        refinedData,
        (datumForTimeStamp, keyForTimeStamp) => {
  ...
  render() {
    return (
      <div>
        ...
        <div>
          <button onClick={() => this.props.dispatch(getTradeData())}>
            새로운 데이터 가져오기
          </button>
        </div>
      </div>
